...
class TypeNode(ExprNode):
    ...
    def __init__(self, name: IdentNode, map_types: Tuple[ExprNode, ExprNode] = (),
                 row: Optional[int] = None, col: Optional[int] = None, **props) -> None:
        super().__init__(row=row, col=col, **props)
        self.name = name
        self.type = None
    ...
    def __str__(self) -> str:
        ...
        if len(self.map_types) > 0:
            map_str += "<" + str(self.map_types[0]) + ", " + str(self.map_types[1]) + ">"

        return str(self.name) + map_str
    ...
    def semantic_check(self, scope: IdentScope) -> None:
        ...
        if self.type == TypeDesc.MAP:
            if len(self.map_types) != 2 or self.map_types[0] is None or self.map_types[1] is None:
                self.semantic_error('Неизвестный тип словаря')
            else:
                self.map_types[0].semantic_check(scope)
                self.map_types[1].semantic_check(scope)
        else:
            if len(self.map_types) != 0:
                self.semantic_error('Использование составного типа для простого')
